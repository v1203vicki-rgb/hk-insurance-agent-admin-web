...
export default async function AdminPage() {
  ...
  try {
    ...
    reviewRows = documentResponse.items.slice(0, 4).map((item) => [
      item.fileName,
  ...
  return (
    <PageShell
      title="平台管理后台"
      description="公共知识库、资料审核、Agent 规则与合规控制的统一工作台。"
      actions={
        <>
    ...
    >
      ...
      <section style={{ display: "grid", gap: 18, gridTemplateColumns: "1.35fr 0.95fr" }}>
        <InfoCard title="资料审核队列" description="审核通过后进入平台公共知识库。">
          <DataTable headers={["文件", "来源公司", "来源等级", "审核状态", "操作"]} rows={reviewRows} gridTemplateColumns="2fr 1.2fr 0.9fr 0.9fr 0.7fr" />
        </InfoCard>
        ...
        <InfoCard title="Agent 合规规则">
          <div style={{ display: "grid", gap: 12 }}>
            {[
              ...
              <div key={rule} style={{ padding: "14px 16px", borderRadius: 18, background: "#f4f8fe", color: "#172036", lineHeight: 1.6 }}>
                {rule}
              </div>
            ))}
          </div>
      ...
      <section style={{ display: "grid", gap: 18, gridTemplateColumns: "1fr 1fr" }}>
        <InfoCard title="租户管理" description="平台管理员可查看租户数量、经纪人数量和问答量。">
          <DataTable headers={["经纪公司", "经纪人", "问答量", "状态"]} rows={tenants.map(([name, brokers, asks, status]) => [name, brokers, asks, <StatusBadge key={`${name}-status`} label={status} />])} gridTemplateColumns="1.8fr 0.8fr 0.8fr 0.8fr" />
        </InfoCard>

        <InfoCard title="来源可信度" description="回答生成时按来源等级优先级排序。">
          <div style={{ display: "grid", gap: 12 }}>
            {[
              ["L1 官方条款", "最高优先级"],
              ["L2 官方小册子", "高优先级"],
              ["L3 费率 / 利益说明", "收益类需重点提示"],
              ["L4 官方通告 / 优惠", "需检查有效期"],
              ["L5 内部培训资料", "必须提示以官方条款为准"],
            ].map(([title, desc]) => (
              <div key={title} style={{ padding: "14px 16px", borderRadius: 18, background: "#f4f8fe", display: "flex", justifyContent: "space-between", gap: 12 }}>
                <strong>{title}</strong>
